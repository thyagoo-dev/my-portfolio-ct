document.addEventListener('DOMContentLoaded', () => {
    ...
    function getOrCreateCvModal() {
        let modal = document.getElementById('cv-modal');
        if (modal) return modal;

        const modalMarkup = `
            <div class="modal" id="cv-modal" role="dialog" aria-modal="true" aria-labelledby="cv-modal-title" aria-hidden="true">
                <div class="modal-backdrop" data-close-modal></div>
                <div class="modal-content" role="document">
                    <button class="modal-close" type="button" aria-label="Fechar modal" data-close-modal>
                        <i data-lucide="x"></i>
                    </button>
    ...
    const cvModal = cvButtons.length ? getOrCreateCvModal() : null;

    function openCvModal(triggerButton) {
        if (!cvModal) return;
        lastCvTrigger = triggerButton || null;
        cvModal.setAttribute('aria-hidden', 'false');
        document.body.classList.add('modal-open');
        cvModal.querySelector('.modal-close')?.focus();
    }
    ...
    if (cvModal) {
        cvModal.querySelectorAll('[data-close-modal]').forEach((el) => {
